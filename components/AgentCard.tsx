...
const AgentCard: React.FC<{
    ...
    // Disable if it's a Gemini text agent in offline text mode,
    // OR if it's an image agent and we are NOT in offline image mode (no local model is loaded).
    const isDisabled = 
        (isGeminiTextAgent && isOfflineMode) || 
        (isImageAgent && !isOfflineImageMode);

    const getDisabledTitle = () => {
        if (isGeminiTextAgent && isOfflineMode) {
            return 'This agent requires an online model (Gemini) and is disabled in local GGUF mode.';
        }
